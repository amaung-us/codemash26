...
You will need to login to pi to work remotely.

SSH into the pi and create the ML directory

    ssh pi@192.168.1.xxx
    md ML
    cd ML
 
 In a **different terminal window**, scp (remote transfer file) the **model.tflite** into the ML directory

    scp model.tflite pi@192.168.1.238:/home/pi/ML 

Go back to the original terminal window and create a **lables.txt** file

    nano labels.txt
List your trained results in a list one at a time and save the file (CTRL + X) Y

    panda
    Pi
    none

Update and upgrade, and install neccessary packages for python

    sudo apt-get update
    sudo apt-get install -y python3-opencv python3-numpy python3-venv python3-pip 


Create python environment and activate environment

    python3 -m venv ~/ML/ml --system-site-packages
    source ~/ML/ml/bin/activate

Install pip packages inside of the environment

    python3 -m pip install --upgrade pip
    python3 -m pip install tflite-runtime flask

Create **detect_web_tflite.py** script file

    nano detect_web_tflite.py


Type the following in the file and save (CTRL+X) Y
...
Run the python script and go to website

    python detect_web_tflite.py
...
## Try this for fun:
save the run the following script
...
Run this:
    ...
    python3 color_web.py --fps 2 --width 320 --height 240 --downsample 2
